...
Explicitly **not** built yet: real billing (plan is a manually-set DB flag),
search-engine visibility beyond on-page basics (no verified property, no
indexed content depth, no analytics), and a test suite that exercises the
app the way a user or CI pipeline would (integration/E2E, component tests).
Those three are this document's main focus, plus the smaller items needed
to make the app production-solid.

---

## 1. Stripe billing — real payments and plan access
...
upgrade, downgrade, or cancel themselves. This is the highest-leverage gap:
the product cannot make money until this exists.
...
- [ ] **Webhooks.** `/api/webhooks/stripe` verifying the signature
  ...
  a transactional provider exists). Must be idempotent (Stripe retries
  webhooks; dedupe on event id or make handlers safe to replay).
...
- [ ] **Tax/compliance basics.** Decide whether to enable Stripe Tax
  (or at minimum collect billing address) before charging real customers;
  note that a Terms of Service + refund policy should exist before taking
  payment (see legal items below — this becomes a hard blocker once billing
  is live, not just "nice to have").
...
- [ ] **End-to-end tests.** Add Playwright (already present in this
  environment's tooling conventions) covering the golden paths: signup →
  ...
  Playwright.
- [ ] **Stripe integration tests** (once billing ships, see §1): test-mode
...
- [ ] **Auth security regression tests.** Once login throttling and
  session revocation exist (see §4), add explicit tests: N failed logins
  lock out/backoff correctly; a revoked session's JWT is rejected even
...
- [ ] **Coverage reporting.** Turn on `vitest --coverage` in CI (fails or at
  ...
  assumed.
- [ ] **Fix known dead code flagged by review**, since it's the kind of
  thing a linter/test pass should catch going forward: unused top-level
  constants in `components/ScoreDial.tsx` (`SIZE`, `STROKE`, `RADIUS`,
  `CIRC`, shadowed by per-instance calculations). Trivial, but tracked here
  rather than dropped.

## 4. Other hardening (carried over from FOLLOWUP.md, still open)

Kept here rather than duplicated — see `FOLLOWUP.md` for the original
framing. Re-prioritized: legal items move up because they become blockers
once Stripe billing is live (§1).

- [ ] **Privacy policy + Terms of Service.** No longer optional once real
  payments are collected — needed before enabling Stripe checkout in
  production. Must disclose: what's collected (email, password hash,
  uploaded screenshots), that screenshots are sent to Anthropic's API and
  discarded (not stored), and billing data handling (Stripe as processor).
- [ ] **Login throttling / brute-force protection** on `/api/auth/login` —
  only `/api/audit` is rate-limited today.
- [ ] **Signup account-enumeration** — `/api/auth/signup`'s "account already
  exists" error leaks registered emails. Needs an explicit decision
  (accept the trade-off, or switch to a generic message + email-based
  confirmation).
- [ ] **Session revocation.** Stateless JWTs, 30-day TTL, no server-side
  denylist — logout only clears the client cookie.
- [ ] **Real "forgot password" flow** + **email verification** — both
  blocked on picking a transactional email provider (e.g. Resend, Postmark).
  Worth deciding now since Stripe billing emails (receipts, payment-failed
  notices) raise the same "we need transactional email" need — solve once
  for both rather than twice.
- [ ] **`npm audit` / dependency hygiene** — clean as of the Next 16 bump;
  re-check periodically (this is now doubly important once CI exists, per
  §3 — add `npm audit --audit-level=high` as a CI step rather than a manual
  habit).
...
1. **CI + component/E2E tests (§3)** first — every other change below is
   safer to ship once there's automated coverage and a CI gate.
2. **Stripe billing (§1)** — the product's core unlock; do the legal
   prerequisites (§4: privacy policy, ToS) alongside it since they block
   going live with real charges either way.
3. **SEO foundations (§2: custom domain, Search Console, content hub)** —
   can run in parallel with billing work; content hub is the long lead-time
   item, worth starting early.
4. **Remaining auth hardening (§4)** — important but not launch-blocking
   the way billing/legal are; sequence opportunistically.
